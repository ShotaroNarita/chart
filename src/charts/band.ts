  ...
  for (const row of data.rows) {
    for (const seg of row.segments) {
      if (!colorMap.has(seg.label)) {
        colorMap.set(
          seg.label,
          WAFU_COLORS[hashLabel(seg.label) % WAFU_COLORS.length]
        );
      }
    }
  }
...
export function generateBandChart(
  data: BandData,
  style: StyleConfig
): string {
  ...
  const maxTotal = Math.max(
    ...data.rows.map((r) => r.segments.reduce((sum, s) => sum + s.value, 0))
  );

  const hasTitle = !!data.title;
  const titleAreaHeight = hasTitle ? TITLE_HEIGHT : 0;
  const barsAreaHeight =
    data.rows.length * BAR_HEIGHT + (data.rows.length - 1) * BAR_GAP;
  const legendRows = Math.ceil(legendLabels.length / Math.floor((LABEL_WIDTH + CHART_WIDTH) / LEGEND_ITEM_WIDTH));
  const totalHeight =
    PADDING + titleAreaHeight + barsAreaHeight + PADDING + legendRows * LEGEND_HEIGHT + PADDING;
  ...
  if (hasTitle) {
    const titleText = data.unit
      ...
      : data.title!;
    lines.push(
      `<text x="${totalWidth / 2}" y="${yOffset + 4}" text-anchor="middle" font-size="16" font-weight="bold" font-family="${FONT_FAMILY}" fill="#1f2937">${escapeXml(titleText)}</text>`
    );
  ...
  for (const row of data.rows) {
    ...
    // Row label
    lines.push(
      `<text x="${PADDING + LABEL_WIDTH - 8}" y="${yOffset + BAR_HEIGHT / 2 + 5}" text-anchor="end" font-size="13" font-family="${FONT_FAMILY}" fill="#374151">${escapeXml(row.name)}</text>`
    );
    ...
    for (const seg of row.segments) {
      ...
      lines.push(
        `<rect x="${xOffset}" y="${yOffset}" width="${segWidth}" height="${BAR_HEIGHT}" fill="${escapeXml(color)}" rx="2"/>`
      );

      // Value label inside the segment (only if wide enough)
      if (segWidth > 30) {
        lines.push(
          `<text x="${xOffset + segWidth / 2}" y="${yOffset + BAR_HEIGHT / 2 + 5}" text-anchor="middle" font-size="12" font-family="${FONT_FAMILY}" fill="white">${seg.value}</text>`
        );
      }
    ...
    lines.push(
      `<text x="${xOffset + 6}" y="${yOffset + BAR_HEIGHT / 2 + 5}" text-anchor="start" font-size="12" font-family="${FONT_FAMILY}" fill="#6b7280">${rowTotal}</text>`
    );
  ...
  for (const label of legendLabels) {
    ...
    lines.push(
      `<rect x="${lx}" y="${ly}" width="14" height="14" fill="${escapeXml(color)}" rx="2"/>`
    );
    lines.push(
      `<text x="${lx + 20}" y="${ly + 12}" font-size="12" font-family="${FONT_FAMILY}" fill="#374151">${escapeXml(label)}</text>`
    );
